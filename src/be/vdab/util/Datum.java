...
public class Datum implements IDatum, Comparable<Datum>, Serializable {
    private String fouteDatum;
    private int dag;
    private int maand;
    private int jaar;

    public Datum(int dag, int maand, int jaar) {
        fouteDatum= dag + "/" +maand+"/"+jaar;

        if (checkJaar(jaar)) {
            this.jaar = jaar;
        } else {
            throw new DatumException(fouteDatum + " - Het opgegeven jaar is niet in het juiste bereik (1584 t.e.m. 4099)");
        }
        if (checkMaand(maand)) {
            this.maand = maand;
        } else {
            throw new DatumException(fouteDatum + " - De opgegeven maand klopt niet (1 t.e.m. 12)");
        }

        if (checkDag(dag)) {
            this.dag = dag;
        } else {
            throw new DatumException(fouteDatum + " - De opgegeven dag klopt niet");
        }
